...
function moveCircles(iterations, day_left, current_iteration) {
    ...
    if(isMidWeek(day_left)) {
        ...
        if (isIterationOver(day_left)) { // fin d'itération
            playGong();
            retroTime(iterations, current_iteration);
        } else {
            if (isFirstDay(day_left)) { // début d'itération
                playGong();
            } else {
                playSong();
            }
...
function soundButton(path) {
    return paper.path(path).attr({fill: "#000", stroke: "none"}).hide().translate(770, 575);
}

function soundMgmt() {
    var soundOn = soundButton(soundOnPath);
    var soundOff = soundButton(soundOffPath);

    buttonMgmt(soundOn, soundOff,
      function () {
          sound = !sound;
      }
    );

    showButton(sound, soundOn, soundOff);
}
...
//
// Create & management manual/auto button
//
function textButton(text) {
  return paper.text(0, 0, text).attr("font-size", 12).translate(750, 590).hide();
}
...
function manualMgmt() {
    ...
    buttonMgmt(manualButton, autoButton, 
      function () {
          manual = !manual;
      }
    );

    showButton(manual, manualButton, autoButton);
}

//
// button management
//
function showButton(enable, enabledButton, disabledButton) {
  if (enable) {
    enabledButton.show();
    disabledButton.hide();
  } else {
...
function buttonMgmt(enabledButton, disabledButton, changeStateCallback) {
  enabledButton.click(
      function () {
          enabledButton.hide();
          disabledButton.show();
...
function isFirstDay(day_left) {
  return day_left == (NB_DAYS -1);
}

function isMidWeek(day_left) {
  return day_left == (NB_DAYS/2 - 1);
}

function isIterationOver(day_left) {
  return day_left < 0;
}